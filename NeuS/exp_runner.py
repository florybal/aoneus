...
class Runner:
    def __init__(self, conf_path, mode='train', case='CASE_NAME', is_continue=False, init_opt=True, sdf_network=None, random_seed=0):
        ...
        self.base_exp_dir = f"{self.conf['general.base_exp_dir']}/{random_seed}"
    ...
    def do_one_iter(self, image_idx):
        ...
        variation_loss = F.l1_loss(alpha, torch.zeros_like(alpha))
        ...
        # print(variation_loss)

        loss = color_fine_loss +\
                eikonal_loss * self.igr_weight +\
                mask_loss * self.mask_weight + variation_loss * self.conf["train.variation_weight"]
        self.iter_step += 1
        return loss
    ...
    def train(self):
        ...
        for iter_i in tqdm(range(res_step)):
            loss = self.do_one_iter(image_perm[self.iter_step % len(image_perm)])
