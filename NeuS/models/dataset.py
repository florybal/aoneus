...
class Dataset:
    def __init__(self, conf):
        ...
        self.is_seathru = (
            os.path.exists(os.path.join(self.data_dir, "poses_bounds.npy"))
            and _find_image_dir(self.data_dir) is not None
        )
        ...
        self.render_cameras_name = conf.get_string(
            "render_cameras_name", default="cameras_sphere.npz"
        )
        self.object_cameras_name = conf.get_string(
            "object_cameras_name", default="cameras_sphere.npz"
        )
    ...
    def _load_seathru_dataset(self):
        poses_bounds = np.load(os.path.join(self.data_dir, "poses_bounds.npy")).astype(
            np.float32
        )
        ...
        self.object_bbox_min = np.array(
            [-bbox_radius, -bbox_radius, -bbox_radius], dtype=np.float32
        )
    ...
    def gen_random_rays_at(self, img_idx, batch_size):
        ...
        rays_v = cvt_neus_coords_to_ho_coords(rays_v)
        rays_o = cvt_neus_coords_to_ho_coords(rays_o)
        return torch.cat([rays_o, rays_v, color, mask[:, :1]], dim=-1).to(
            self.device
        )  # batch_size, 10
